...
from .hybrid_encoder import HybridEncoder, RepNCSPELAN4
from ..extre_module.custom_nn.featurefusion.SCEF import SCEF
  
__all__ = ['HybridEncoder_SCEF']
    
@register()
class HybridEncoder_SCEF(HybridEncoder):
    ...
    def forward(self, feats):    
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):   
            ...
            inner_out = self.fpn_scef_blocks[len(self.in_channels) - 1 - idx]([feat_low, feat_heigh])
        ...
        for i, feat in enumerate(inner_outs):
            print(f"编码器输出特征 {i} 形状：{feat.shape}")  # 应输出 (B, hidden_dim, H, W)
        return inner_outs  # 返回融合后的多尺度特征图列表
