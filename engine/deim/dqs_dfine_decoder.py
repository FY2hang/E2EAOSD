...
class DQSDFINETransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 suppress_threshold=0.6,  # 抑制阈值
                 suppress_factor=0.1,     # 抑制因子
                 min_density_score=0.1,    # 允许的最小密度分数，避免0乘
                 ):
        super().__init__()
        ...
        self.min_density_score = min_density_score  # 方法2: 最小密度分数
    ...
    def _select_topk(self, memory: torch.Tensor, outputs_logits: torch.Tensor, densityMapMemory: torch.Tensor, outputs_anchors_unact: torch.Tensor, topk: int):
        ...
        if self.query_select_method == 'default':
            if self.iter < self.using_densitymap_iter:  # 还未使用densitymap
                ...
                if self.iter == self.using_densitymap_iter:
                    ...
                    # logger.info(f"using densityMap[min_score:{self.min_density_score:.2f}] and logits in select topk...") # 方法2: 最小密度分数
                # 步骤 1: 原始的温度处理
                densityMapMemory = densityMapMemory ** (1 / self.densitymap_temperature)
                # 步骤 2: 原始的归一化 (前景FG=1.0, 背景BG=0.6)
                densityMapMemory = densityMapMemory / densityMapMemory.max()
                # === 改进的抑制步骤 ===
                # 创建一个掩码，找到所有低于阈值 (0.7) 的点
                suppress_mask = (densityMapMemory < self.suppress_threshold)
                # 将这些点的值乘以一个很小的抑制因子 (例如 0.1)
                densityMapMemory[suppress_mask] = densityMapMemory[suppress_mask] * self.suppress_factor
                # 步骤 3: 最终乘法，此时 FG = 1.0, BG = 0.6 * 0.1 = 0.06
                _, topk_ind = torch.topk(outputs_logits.max(-1).values * densityMapMemory, topk, dim=-1)
                # # 方法2
                # densityMapMemory = torch.clamp(densityMapMemory, min=self.min_density_score)
                # # (注意：这里我们不需要对 .max() 做 clamp，因为 max 必定 >= min_density_score)
                # densityMapMemory = densityMapMemory / densityMapMemory.max()
                # _, topk_ind = torch.topk(outputs_logits.max(-1).values * densityMapMemory, topk, dim=-1)
