'Bidirectional Diffusion Pyramid Network (BDPN) Neck for Multi-Scale Feature Fusion'
...
class BDPN(nn.Module): 
    def __init__(self,     
                 ...
                 feat_strides=[4, 8, 16],             # 输入特征图的步幅列表，表示特征图相对于输入图像的缩放比例    
                 ...
                 out_strides=[4, 8, 16],              # 输出特征图的步幅列表  n版本只要16和32
        ...
        # --------------------------- 第一阶段    
        self.FocusFeature_1 = BDBlock(inc=[hidden_dim, hidden_dim, hidden_dim], kernel_sizes=fdpn_ks)
    
        # P3 → P4 路径（下采样到更低分辨率）
        self.p3_to_p4_down1 = Conv(hidden_dim, hidden_dim, k=3, s=2)   
        self.p4_block1 = C2f(hidden_dim * 2, hidden_dim, round(3 * depth_mult), shortcut=True)    

        # P3 → P2 路径（上采样到更高分辨率）
        self.p3_to_p2_up1 = nn.Upsample(scale_factor=2)
        self.p2_block1 = C2f(hidden_dim * 2, hidden_dim, round(3 * depth_mult), shortcut=True)  
        
        # --------------------------- 第二阶段 
        self.FocusFeature_2 = BDBlock(inc=[hidden_dim, hidden_dim, hidden_dim], kernel_sizes=fdpn_ks)

        # P3 → P4 路径（第二次下采样）
        self.p3_to_p4_down2 = Conv(hidden_dim, hidden_dim, k=3, s=2) 
        self.p4_block2 = C2f(hidden_dim * 3, hidden_dim, round(3 * depth_mult), shortcut=True)     
    
        # P3 → P2 路径（第二次上采样）
        if 4 in out_strides:  # 如果输出包含P2
            self.p3_to_p2_up2 = nn.Upsample(scale_factor=2)
            self.p2_block2 = C2f(hidden_dim * 3, hidden_dim, round(3 * depth_mult), shortcut=True)   
    ...
    def forward(self, feats):   
        ...
        # 第一阶段：以P3为中心的特征融合
        # 注意：FocusFeature期望输入顺序为[大尺度, 中尺度, 小尺度] -> [P4, P3, P2]
        focus_feature1 = self.FocusFeature_1([proj_feats[2], proj_feats[1], proj_feats[0]])  # [P4, P3, P2]

        # P3 → P4 路径（下采样）
        focus_feature1_to_p4_1 = self.p3_to_p4_down1(focus_feature1) 
        focus_feature1_to_p4_2 = self.p4_block1(torch.cat([focus_feature1_to_p4_1, proj_feats[2]], dim=1))

        # P3 → P2 路径（上采样）
        focus_feature1_to_p2_1 = self.p3_to_p2_up1(focus_feature1) 
        focus_feature1_to_p2_2 = self.p2_block1(torch.cat([focus_feature1_to_p2_1, proj_feats[0]], dim=1))    

        # 第二阶段：进一步细化特征
        # 输入顺序：[P4, P3, P2] （从第一阶段的结果）
        focus_feature2 = self.FocusFeature_2([focus_feature1_to_p4_2, focus_feature1, focus_feature1_to_p2_2]) 
    
        # P3 → P4 路径（第二次下采样）
        focus_feature2_to_p4 = self.p3_to_p4_down2(focus_feature2) 
        focus_feature2_to_p4 = self.p4_block2(torch.cat([
            focus_feature2_to_p4, 
            focus_feature1_to_p4_1, 
            focus_feature1_to_p4_2
        ], dim=1))   

        # P3 → P2 路径（第二次上采样）
        if 4 in self.out_strides:
            focus_feature2_to_p2 = self.p3_to_p2_up2(focus_feature2) 
            focus_feature2_to_p2 = self.p2_block2(torch.cat([
                focus_feature2_to_p2, 
                focus_feature1_to_p2_1, 
                focus_feature1_to_p2_2
            ], dim=1))
            return [focus_feature2_to_p2, focus_feature2, focus_feature2_to_p4]  # P2, P3, P4
        else:    
            return [focus_feature2, focus_feature2_to_p4]  # P3, P4
