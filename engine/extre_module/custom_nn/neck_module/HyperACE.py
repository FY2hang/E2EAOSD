'''
本文件由BiliBili：魔傀面具整理
engine/extre_module/module_images/HyperACE.md
...
from engine.extre_module.ultralytics_nn.block import C3, C2f
 
__all__ = ['DSConv_YOLO13', 'DSC3k2', 'HyperACE', 'DownsampleConv', 'FullPAD_Tunnel']
...
if __name__ == '__main__':
    ...
    print(RED + '-'*20 + " HyperACE " + '-'*20 + RESET)
  
    feats = [inputs_P3, inputs_P4, inputs_P5] 
    module = HyperACE(channel, channel, n=1, num_hyperedges=8, dsc3k=True, shortcut=True, channel_adjust=False).to(device)    
    ...
    print(RED + '-'*20 + " FullPAD_Tunnel " + '-'*20 + RESET)    

    inputs_1 = torch.randn((batch_size, channel, height, width)).to(device)  
    inputs_2 = torch.randn((batch_size, channel, height, width)).to(device)
    
    feats = [inputs_1, inputs_2] 
    module = FullPAD_Tunnel().to(device)   
    
    outputs = module(feats)
    print(GREEN + f'inputs_1.size:{inputs_1.size()} inputs_2.size:{inputs_2.size()} outputs.size:{outputs.size()}' + RESET)
    ...
    print(RED + '-'*20 + " DownsampleConv " + '-'*20 + RESET)
    inputs = torch.randn((batch_size, channel, height, width)).to(device)

    module = DownsampleConv(channel).to(device)    
