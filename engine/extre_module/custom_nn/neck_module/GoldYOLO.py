''' 
本文件由BiliBili：魔傀面具整理
engine/extre_module/module_images/NeurIPS2023-GOLDYOLO.png
engine/extre_module/module_images/NeurIPS2023-GOLDYOLO.md
...
__all__ = ['SimFusion_4in', 'SimFusion_3in', 'IFM', 'InjectionMultiSum_Auto_pool', 'InjectionMultiSum_Auto_pool_Fourier', 
...
class InjectionMultiSum_Auto_pool_Fourier(nn.Module):     
    def __init__(    
            self,
            inp: int,     
            oup: int,   
        ...
        self.local_embedding = Conv(inp, oup, 1, act=False)  
        # self.global_embedding = Conv(global_inp[self.flag], oup, 1, act=False)
        self.global_embedding = FourierConv(global_inp[self.flag], oup, size=size, act=False)
        self.global_act = Conv(global_inp[self.flag], oup, 1, act=False)    
        self.act = h_sigmoid()
...
if __name__ == '__main__':
    ...
    inputs_P2 = torch.randn((batch_size, channel, height * 8, width * 8)).to(device)    
    ...
    print(RED + '-'*20 + " SimFusion_4in " + '-'*20 + RESET)
   
    feats = [inputs_P2, inputs_P3, inputs_P4, inputs_P5]   
    module = SimFusion_4in().to(device)
    ...
    print(RED + '-'*20 + " SimFusion_3in " + '-'*20 + RESET) 

    feats = [inputs_P3, inputs_P4, inputs_P5] 
    module = SimFusion_3in([channel, channel, channel], channel).to(device)     
    ...
    print(RED + '-'*20 + " IFM " + '-'*20 + RESET) 
   
    module = IFM(channel, [64, 32]).to(device)
    
    outputs = module(inputs_P5)
    print(GREEN + f'inputs.size:{inputs_P5.size()} outputs.size:{outputs.size()}' + RESET)
    ...
    print(RED + '-'*20 + " InjectionMultiSum_Auto_pool " + '-'*20 + RESET)

    feature = torch.randn((batch_size, 64 + 32, height, width)).to(device) 
    module = InjectionMultiSum_Auto_pool(channel, channel, [64, 32], 0).to(device) 

    feats = [inputs_P5, feature]     
    outputs = module(feats)
    print(GREEN + f'inputs.size:{inputs_P5.size()} outputs.size:{outputs.size()}' + RESET) 
    ...
    print(RED + '-'*20 + " InjectionMultiSum_Auto_pool_Fourier " + '-'*20 + RESET)  

    feature = torch.randn((batch_size, 64 + 32, height, width)).to(device)     
    module = InjectionMultiSum_Auto_pool_Fourier(channel, channel, [64, 32], 0, size=(height, width)).to(device)   
    ...
    print(RED + '-'*20 + " PyramidPoolAgg " + '-'*20 + RESET)

    feats = [inputs_P3, inputs_P4, inputs_P5]     
    module = PyramidPoolAgg(sum([channel, channel, channel]), channel, 2).to(device)    
    ...
    print(RED + '-'*20 + " TopBasicLayer " + '-'*20 + RESET)
    
    module = TopBasicLayer(channel, [64, 128]).to(device)
    ...
    print(RED + '-'*20 + " AdvPoolFusion " + '-'*20 + RESET)
 
    feats = [inputs_P4, inputs_P5]
    module = AdvPoolFusion().to(device)     
 
    outputs = module(feats)
    print(GREEN + f'inputs_P4.size:{inputs_P4.size()} inputs_P5.size:{inputs_P5.size()} outputs.size:{outputs.size()}' + RESET)     
